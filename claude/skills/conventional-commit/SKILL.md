...
3. Extract the Jira ticket ID from the branch name: everything before the first underscore (e.g., `PROJ-123` from `PROJ-123_add_user_validation`). If no underscore or no ticket pattern found, omit the ticket tag.
...
5. Determine scope from the primary area of change (e.g., `auth`, `api`, `users`).
...
- Example: `fix(auth): [PROJ-123] add input validation to login endpoint`
